...
class Framework:
    ...
    @staticmethod
    def get_feature_tensor(X: torch.FloatTensor) -> torch.FloatTensor:
    ...
    @staticmethod
    def get_label_tensor(y: torch.LongTensor) -> torch.LongTensor :
    ...
    @staticmethod
    def get_mask_tensor(mask: torch.BoolTensor) -> torch.BoolTensor :
    ...
    @staticmethod
    def get_edge_index_tensor(edge_index: torch.LongTensor) -> torch.LongTensor:
    ...
    def feature_importance_per_class(self, class_idx, n_repeats=1):
        ...
        if self.multi_target_class:
            ...
            if class_idx:
                return np.mean([softmax(np.abs(framework.trained_clfs[i].estimators_[class_idx].coef_)) for i in
                                range(len(framework.trained_clfs))], axis=0)
            return np.mean([softmax(np.abs(framework.trained_clfs[i].estimators_[:].coef_)) for i in
                            range(len(framework.trained_clfs))], axis=0)
        if not self.multi_target_class:
            if class_idx:
                return np.mean([softmax(np.abs(framework.trained_clfs[i].coef_[class_idx])) for i in
                                range(len(framework.trained_clfs))], axis=0)
            return np.mean(
                [softmax(np.abs(framework.trained_clfs[i].coef_)) for i in range(len(framework.trained_clfs))], axis=0)
    ...
    def feature_importance(self, n_repeats=10):
        ...
        if not self.multi_target_class:
            ...
            if is_linear_clfs:
                return softmax(
                    np.mean([framework.trained_clfs[i].coef_[0] for i in range(len(framework.trained_clfs))], axis=0))
        if self.dataset is None: raise Exception(
